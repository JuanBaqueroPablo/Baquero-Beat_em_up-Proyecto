...
func _perseguir():
	if atacando or recibiendo_daño or muriendo:
		velocity.x = 0
		return
	var direccion = (jugador.global_position - global_position).normalized()
	velocity.x = direccion.x * velocidad
	animaciones.play("Camina-Esqueleto")
	animaciones.flip_h = jugador.global_position.x < global_position.x

func _atacar():
	atacando = true
	velocity.x = 0
	animaciones.play("Ataque-esqueleto")
	animaciones.flip_h = jugador.global_position.x < global_position.x
func recibir_daño(cantidad):
	...
	if vida_actual <= 0:
		...
		animaciones.play("Muerte-esqueleto")
	else:
		atacando = false
		recibiendo_daño = true
		animaciones.play("Danio-esqueleto")
...
func _on_animacion_terminada():
	if muriendo:
		...
		recibiendo_daño = false
		animaciones.play("Quieto-esqueleto")
	elif atacando:
		atacando = false
